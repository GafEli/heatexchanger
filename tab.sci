function [t_hot_out, t_cold_out, n_iter, Tc, Th] = tab(t_cold_in, t_hot_in, f);
    // 26-04-2023
//    t_cold_in = 15; // Входная температура холодного потока (хладагента)
//    t_hot_in = 70; // Входная температура горячего потока
    if  ~(t_cold_in < t_hot_in) then // обработка возможной ошибки ввода
...
//    f = 32.2; // поверхность теплообмена м2
    ...
    while n_iter < iterations // q_f>q_exch // итерации по температуре хладагента
        ...
        if t_h < t_cold_in then // в процессе выполнения итераций возможно t_h>t_cold_in, коррекция:
        ...
        if abs(delta_q)<eps then // Если она меньше допустимой погрешности, 
            break; // выйти из цикла;
        elseif delta_q>0 then // иначе, если она положительна, решение левее;
            t_c_b = t_c; // в этом слуае подвинуть правую границу влево.
    ...
    plot(Q_f,'r','thickness',3); // график потока через поверхность теплообмена, W, от номера итерации
    figure; plot(Tc,'b','thickness',3); // график искомой выходной температуры хладагента от номера итерации
    plot(Th,'r','thickness',3); // график искомой выходной температуры горячего потока
endfunction; // Конец основной программы
...
// Данная функция является симметричной: $\textrm{log_mean}(z_1,z_2)=\textrm{log_mean}(z_2,z_1)$
    if z1==z2 then // если усредняемые числа равны, ...
...
endfunction; // Конец подпрограммы вычисления среднего логарифмического температурного напора
