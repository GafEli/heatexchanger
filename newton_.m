function newton_() % реализация метода Ньютона
   ...
   hinv2 = 1/(2*h); % множитель для вычисления производной
    ...
    while abs(dx) > eps_max % пока ошибка превышает допустимую,
        ...
        dy = f1(x,h,hinv2); % значение производной в текущей точке
...
function dy1=f1(x1,hloc,hinvloc) % подпрограмма вычисления производной
    dy1 = hinvloc*(f0(x1+hloc)-f0(x1-hloc)); % значение производной в точке
